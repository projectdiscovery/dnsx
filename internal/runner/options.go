...
func ParseOptions() *Options {
	...
	flagSet.CreateGroup("configs", "Configurations",
		...
		flagSet.BoolVarP(&options.AutoWildcard, "auto-wildcard", "aw", false, "automatic wildcard filtering across all input domains (mutually exclusive with -wd)"),
...
// validateAutoWildcardFlags returns an error when mutually-exclusive wildcard
// flags are combined.  Keeping this as a standalone function makes it directly
// unit-testable without triggering gologger.Fatal.
func (options *Options) validateAutoWildcardFlags() error {
	if options.AutoWildcard && options.WildcardDomain != "" {
		return errors.New("auto-wildcard (-aw) and wildcard-domain (-wd) flags are mutually exclusive")
	}
	return nil
}
...
func (options *Options) validateOptions() {
	...
	if err := options.validateAutoWildcardFlags(); err != nil {
		gologger.Fatal().Msgf("%s\n", err)
	}
